...
def make_dataframe_with_eos_mask(test_df :pd.DataFrame,args) -> (dict,dict):
...
def main(args):
    ...
    test = make_dataframe_with_eos_mask(test_df,args)
    ...
    tf.data.experimental.save(test_q_dataset, args.tf_data_dir+'/total_q/test')
    tf.data.experimental.save(test_q_dataset, args.tf_data_dir+'/total_c/test')
