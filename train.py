...
def evaluate(model,test_dataset,test_summary_writer,config_xl):
    ...
    for input_data_q,input_data_c, test_mask, test_labels in tqdm(test_dataset, desc='eval'):

        outputs = model(concepts=input_data_q, responses=test_mask, labels=test_labels, mems=test_mems, training=False)
        loss = outputs.loss
...
def train(train_dataset,train_summary_writer,config_xl):
    try:
        ...
        for epoch in range(config_xl.epoch):
            ...
            for input_data_q,input_data_c, mask, labels in tqdm(train_dataset, desc='train'):
                mems, loss_value = train_step(model, input_data_q,mask, labels, mems, optimizer)
