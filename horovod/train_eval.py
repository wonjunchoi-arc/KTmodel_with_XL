...
class TrainDataGenerator:
    def __init__(self, train_path, bsz, bptt, eos_token, mask_token, ext_len=None):
        self.train_path = train_path.decode('utf-8') 
        self.bsz = bsz
        self.bptt = bptt
...
class TestDataGenerator:
    def __init__(self, test_path, bsz, bptt, eos_token, mask_token, ext_len=None):
        self.test_path = test_path.decode('utf-8')
        self.bsz = bsz
        self.bptt = bptt
        self.eos_token = eos_token
        self.mask_token = mask_token
    ...
    def generate(self):
        ...
        for i in range(0, len(cseq_reshaped[1]) - 1, self.bptt):
            
            seq_len = min(self.bptt, cseq_reshaped.shape[1] - 1 - i) # # i값이 103227020를 넘지 않는 이상 seq_len = 70
            ...
            if i + self.bptt < cseq_reshaped.shape[1] - 1:
                yield C, masked_R,labels
            # yield second_half_data, second_half_target
...
for epoch in range(2):
    ...
    if hvd.rank() == 0:
        test_dataset0 = iter(test_first_half_dataset)       # 첫 번째 데이터셋
        test_mems0 = None       
        test_loss0,test_acc0 = evaluate(model0,test_mems0, test_first_half_dataset)
        print(f'Test Loss on First Half Dataset after Epoch {epoch + 1}: {test_loss0}')
    
    elif hvd.rank() == 1:
        test_dataset1 = iter(test_second_half_dataset)       # 첫 번째 데이터셋
        test_mems1 = None  
        test_loss1, test_acc1 = evaluate(model1,test_mems1, test_second_half_dataset)
        print(f'Test Loss on Second Half Dataset after Epoch {epoch + 1}: {test_loss1}')
...
# horovodrun -np 2 -H localhost:2 python /home/jun/workspace/KT/train_eval.py 실행 명령어
