...
class TransformerXLTrainer:
    ...
    @tf.function
    def train_step(self,model, data1,data2, target, mems, optimizer) -> (list, tf.Tensor,tf.Tensor):
        with tf.GradientTape() as tape:
            ...
            logit_value = tf.reshape(logit_value, [-1, 2])
        ...
        print('mems:',type(mems),type(mean_loss),type(logit))
        
        return mems,mean_loss,logit
        ...
        try:
            ...
            for epoch in range(self.config_xl.epoch):
                ...
                for input_data, masked_responses, responses in tqdm(train_dataset.take(2), desc='train'):
                    mems, loss_value,logit = self.train_step(model, input_data,masked_responses, responses, mems, optimizer)
                    num_batches += 1
    ...
    def run(self):
        ...
        logging.info('config_xl:  %s',self.config_xl)
        
        train_dataset,test_dataset,dkeyid2idx=self.load_TFdataset()
        model = self.train_test(train_dataset,test_dataset)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--d_embed', type=int, required=False, default=512, help='Dimensionality of the embeddings')#baseline 256, 
    parser.add_argument('--d_head', type=int, required=False,default=32,help='Dimensionality of the model’s heads')
    parser.add_argument('--d_model', type=int, required=False,default=512 , help='Dimensionality of the model’s hidden states.') #baseline 256, 
    ...
    parser.add_argument('--n_layer', type=int, required=False, default=4, help='Number of hidden layers in the Transformer encoder') # baselime  4
    ...
    parser.add_argument('--devices', type=str, required=False, default='cpu')
    ...
    parser.add_argument('--mlflow_set_experiment', type=str, required=False, default='TEST',help='Determine which experiment in the mlflow to record the models training records')
    parser.add_argument('--registered_model_name', type=str, required=False, default='TEST',help='The registry name on which the model will be stored')
